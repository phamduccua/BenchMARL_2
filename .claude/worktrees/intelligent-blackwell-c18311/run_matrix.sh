...
usage() {
    ...
    echo "          ippo_extragradient_adaptive | ippo_extragradient_adaptive_nu05"
    echo "          ippo_extragradient_adaptive_ms001 | ippo_extragradient_adaptive_ms05"
    echo ""
    echo "Ví dụ:"
    ...
    for i in "${!TASKS[@]}"; do
        ...
        case "$algo" in
            ...
            ippo_extragradient_adaptive_nu05)
                run_algo "ippo_extragradient_adaptive" "$task_path" "${prefix}_ippo_extragradient_adaptive_nu05" \
                    "algorithm.extra_nu=0.5"
                ;;
            ippo_extragradient_adaptive_ms001)
                run_algo "ippo_extragradient_adaptive" "$task_path" "${prefix}_ippo_extragradient_adaptive_ms001" \
                    "algorithm.extra_min_scale=0.01"
                ;;
            ippo_extragradient_adaptive_ms05)
                run_algo "ippo_extragradient_adaptive" "$task_path" "${prefix}_ippo_extragradient_adaptive_ms05" \
                    "algorithm.extra_min_scale=0.5"
                ;;
            ippo_extragradient_adaptive)
                run_algo "ippo_extragradient_adaptive" "$task_path" "${prefix}_ippo_extragradient_adaptive"
                ;;
            *)
                run_algo "$algo" "$task_path" "${prefix}_${algo}"
                ;;
